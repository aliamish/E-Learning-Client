...
const EditFaq = (props: Props) => {
  ...
  const toggleQuestion = (id: any) => {
    setQuestions((prevQuestion) =>
      prevQuestion.map((q) => (q.id === id ? { ...q, active: !q.active } : q))
    );
  };

  const handleQuestionChange = (id: any, value: string) => {
    setQuestions((prevQuestion) =>
      prevQuestion.map((q) => (q.id === id ? { ...q, question: value } : q))
    );
  };

  const handleAnswerChange = (id: any, value: string) => {
    setQuestions((prevQuestion) =>
      prevQuestion.map((q) => (q.id === id ? { ...q, answer: value } : q))
    );
  };
  ...
  // Function to chek if tha FAQ arrays are unchanged

  const areQuestionsUnchanged = (
    originlQuestions: any[],
    newQuestions: any[]
  ) => {
    return JSON.stringify(originlQuestions) === JSON.stringify(newQuestions);
  };
      ...
      {isLoading ? (
        ...
        <div className="w-[90%] 800px:w-[80%] m-auto mt-[120px]">
          ...
          <div
            className={`${
              styles.button
            } !w-[100px] !min-h-[40px] !h-[40px] dark:text-white bg-[#cccccc34] 
              ${
                areQuestionsUnchanged(data?.layout?.faq, questions) ||
                isAnyQuestionEmpty(questions)
                  ? "!cursor-not-allowed"
